...
class MainViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.speechButton?.setTitle("Pressione aqui para falar", for: .normal)
...
extension MainViewController: WatsonDelegate {
    func didChangeState(to newState: WatsonState) {
        DispatchQueue.main.async {
            ...
            case .listening:
                self.speechButton?.setTitle("Ouvindo...", for: .normal )
                self.watsonImage?.image = self.wQuestion
            case .classifying:
                self.speechButton?.setTitle("Pensando...", for: .normal)
                self.watsonImage?.image = self.wThink
            case .synthesizing:
                self.speechButton?.setTitle("Pensando...", for: .normal)
                self.watsonImage?.image = self.wThink
            case .speaking:
                self.speechButton?.setTitle("Respondendo...", for: .normal)
